...
export const toolDescriptionsVi: BlogPost = {
	...
	discussionNumber: 18,
	discussionUrl: 'https://github.com/TranQui004/signalint-site/discussions/18',
	sections: [
		{
			...
			paragraphs: [
				...
				'Tool Definition Quality được tính bằng công thức 60% trung bình TDQS cộng 40% điểm thấp nhất của các công cụ. Phần 40% điểm thấp nhất đồng nghĩa với việc chỉ một công cụ mô tả kém cũng kéo điểm toàn server xuống, bất kể các công cụ còn lại tốt đến đâu. Mỗi công cụ được chấm 1–5 điểm trên sáu tiêu chí với trọng số khác nhau: Purpose Clarity (độ rõ của mục đích, 25%), Usage Guidelines (hướng dẫn khi nào nên dùng, 20%), Behavioral Transparency (tính minh bạch về tác dụng phụ và cách xử lý lỗi, 20%), Parameter Semantics (ý nghĩa và ràng buộc của tham số, 15%), Conciseness & Structure (độ ngắn gọn và cấu trúc, 10%), và Contextual Completeness (đầy đủ ngữ cảnh để gọi đúng, 10%).',
		...
		},
		{
			...
			paragraphs: [
				...
				'Mô tả mới, trích nguyên văn từ src/index.ts trong repository signalint, như sau:',
			],
		...
		},
		{
			...
			paragraphs: [
				...
				'"Read-only; no files are written or modified" và "absolute paths or paths outside the root return an error response": Behavioral Transparency, tiêu chí nhất quán chỉ được 2 đến 3/5 trên toàn bộ các công cụ trước khi viết lại.',
		...
		},
		{
			...
			paragraphs: [
				...
				'Sáu tiêu chí của TDQS: Purpose Clarity, Usage Guidelines, Behavioral Transparency, Parameter Semantics, Conciseness & Structure, Contextual Completeness. Chúng phản ánh đúng những gì agent cần biết trước khi quyết định gọi công cụ nào và truyền tham số gì. Rubric không thưởng điểm vì mô tả dài, mà thưởng vì mô tả trả lời đủ các câu hỏi đó một cách nhất quán và có thể kiểm chứng.',
