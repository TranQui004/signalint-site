...
export const toolsVi: DocsPageContent = {
	...
	eyebrow: 'MCP schema',
	sections: [
		{ title: 'Quy tắc đầu vào', paragraphs: ['Tool argument từ chối property lạ. Path array nhận tối đa 512 string tương đối; absolute path, path thoát project, NUL và leading dash đều bị từ chối. Toàn bộ 5 công cụ đều khai báo outputSchema và trả về structuredContent.'], note: 'Mọi path được resolve qua một containment boundary chung trước khi bất kỳ adapter nào thấy chúng.' },
		...
		{ title: 'check_files', paragraphs: ['Kiểm tra danh sách file tương đối. Oxlint/Biome chỉ nhận cache miss; tsc dùng file để quyết định có cần chạy, nhưng khi chạy luôn nhìn thấy toàn project. Kích hoạt cảnh báo churn khi một cặp (file, rule) lặp lại qua 3+ lần check.'], code: '{ "files": ["src/index.ts", "src/config.ts"] }' },
		...
		{ title: 'get_loop_status', paragraphs: ['Cho biết chữ ký lint/type/test nào bị dao động (looping) hoặc cặp file/rule nào bị lặp lại (churning) qua 3+ lần gọi check_files trong phiên.'], code: '{}' },
		{ title: 'CheckResponse · schema 1.2', codeLabel: 'JSON', code: '{\n  "schemaVersion": "1.2",\n  "engines": {\n    "oxlint": { "status": "ok" },\n    "tsc": { "status": "ok" },\n    "biome": { "status": "disabled" }\n  },\n  "clusters": [],\n  "truncated": false,\n  "loopWarning": null,\n  "fileRuleChurnWarning": null\n}' },
		...
		{ title: 'Exceptional responses', paragraphs: ['Response schema 1.2 bảo toàn kết quả từ engine đã hoàn thành. Engine timeout hoặc vượt output limit có status error và message riêng dưới engines.'] },
